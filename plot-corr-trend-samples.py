...
for nfr in nframes:
    ...
    rep_msm_rmse = []
    for rep in reps:
        top = Path(f't4l-{rep}')/f'binding/{resect_tag}'
...
fig, ax = plt.subplots()
xticks = nframes
ax.errorbar(nframes, msm_corr, yerr=msm_corr_err, label='MSM binding corr.')
ax.errorbar(nframes, msm_rho, yerr=msm_rho_err, label='MSM binder rho')
ax.errorbar(nframes, best_corr, yerr=best_corr_err, label='Best score corr.')
ax.errorbar(nframes, best_rho, yerr=best_rho_err, label='Best score rho')
ax.set_xticks(xticks)
ax.legend()
ax.set_title(f'Correlation with experiment vs bin samples across {nreps} full replicas')
ax.set_ylabel('Correlation')
# ax = axs[1]
ax.errorbar(nframes, msm_rmse, yerr=msm_rmse_err, label='MSM RMSE')
ax.errorbar(nframes, best_rmse, yerr=best_rmse_err, label='Best score RMSE')
# ax.set_xticks(xticks)
...
fig.savefig('corr-trend-normalize.svg', transparent=True)
fig.savefig('corr-trend-normalize.pdf', transparent=True)
plt.show()
